...
```{r}
datafile$Squirrel<-as.factor(datafile$Squirrel)
datafile$Handed<-as.factor(datafile$Handed)
datafile$Chirality<-as.factor(datafile$Chirality)
...
```{r}
sd(datafile$Width)/sqrt(length(datafile$Width))
```
...
```{r}
plot(datafile$Width, datafile$Time)
```
...
```{r}
{plot(datafile$Width, datafile$Time)
abline(lm(Time~Width, data = datafile))}
```
...
```{r}
model<-lm(Time~Width, data = datafile)
```
...
```{r}
model2<-lm(log(Time)~Width, data = datafile)
hist(resid(model2))
```
