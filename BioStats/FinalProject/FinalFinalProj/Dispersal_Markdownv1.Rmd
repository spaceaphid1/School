...
Title: Analysis of Dispersal Plasticity Under Three Environmental Stress Treatments
Author: Jackson Anderson
Date: Dec. 3rd, 2020
Emery Lab, University of Colorado, Boulder


1) Background:
Over the course of the 2019 Fall semester, the students of Nancy Emery's Evolutionary Ecology class (EBIO 4450) conducted and experiment where they subjected a plethora of l. fremontii plants to varying forms for environmental stress. This study was conducted at the CU Greenhouse on 30th Street. Three different experiments were run for this class, all of them looking at how the manipulation of environmental parameters affected plant biomass, the ratio of dispersive to non-dispersive seeds produced by the maternal plant, and the height of the inflorescence, measured from the base of the plant. One experiment, called the "shade" experiment, subjected groups of plants to different levels of light. Another experiment, the "density" experiment, planted 1 or two plants per cone as a measure of competition. The final experiment, the "resource" experiment, subjected plants to varying degrees of available nutrients, another measure of environmental stress. Data was collected near the end of the semester, and each experimental team analyzed their data, with help from Dr. Emery and those that worked in her lab. After the semester ended, as a member of Dr. Emery's lab, was given the opportunity to perform a meta analysis on all the data collected from each experiment, as well as execute more rigorous testing than we had time for during the class itself. Suffice it to say that the results from this analysis differed substantially from those of the actual class. 

Much of the data parsing was done early in the Fall semester; preliminary visualization and modeling was done during mid Fall semester, and these results were interpreted with more scrutiny over the past few days. Im bummed that I was not able to use my original data set for this assignment, and I learned the hard way to make sure the data fits the assumptions of the models needed to perform the analysis. As well, it should be noted that this data set is somewhat incomplete, as there is survivorship data on some plants from the density experiment have yet to be tallied. When the pandemic hit, it became very difficult to coordinate between my former classmates that ran this experiment, as some left the country to do fieldwork in Norway. Thus, survivorship status was taken into account, when possible. All-in-all, this analysis is still a work-in-progress, and may end up becoming part of my senior thesis, should time permit. Enjoy!!

Experimental Assumptions: Really, the only assumptions made for these experiments is that plant biomass is a measure of stress. The use of inflorescence height was statistically tested for as a dispersal trait by the class, and the seed type proportion as a measure of dispersal propensity is widely supported by cited literature.
...
```{r}
#'Loading the Data
dat <- read_csv("/Users/jacksonanderson/Library/Mobile Documents/com~apple~CloudDocs/HomeBase/DisperalDataOrigFiles/DispersalMasterData_with_weight.csv")
...
```
*It should be noted that extensive diagnostics were run on the response variables for each treatment. In all cases, the distributions were very close to being not normal, even with log transformations. In most cases, log transformation did not solve the issues of underlying non-normality at all. As a result, under the instruction of my boss, I used a model that accounted for underlying heteroskedasticity in its framework, when needed, and I had much more success with these models than I did with log transformation, at least as far as residual variance structures are concerned. The data was not perfect, far from good, in fact. But I made due with what I had! I did not include these diagnostics in this markdown, for the sake of this assignment, as it added approximately 200 lines to an already monstrous script. Andrew or Chris, should you want to see these diagnostics I have a separate script ready to upload where I compared transformed to un-transformed response variable distributions/normality. Let me know if you want this, and I will get it to you ASAP.*
...
```
#9) Density Experiment: Ray seed proportion
...
p_dens_propn_output
```
#10) Resource Experiment: Dispersal Propensity
...
p_res_propn_output
```
#11) Shade Experiment: Inflorescence Height
